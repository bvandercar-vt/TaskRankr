...
- **Changelog & Version Tracking**: A "What's New" dialog automatically appears when users open the app after an update with new changelog entries. Users can also view the full changelog from Settings. Version number is displayed at the bottom of Settings. Changelog content lives in `CHANGELOG.json` at the project root — add new entries at the top of the array. Logic and utilities are in `client/src/lib/changelog.ts`.
