...
- **Task State & Mutations**: Components consume task data and mutations directly from `useLocalState()` (e.g. `const { tasks, updateTask, deleteTask } = useLocalState()`). There is no `useTasks` / `useTaskActions` indirection — `LocalStateProvider` is the single source of truth. All mutations are synchronous local-first writes (no `useMutation`, no `isPending`); `SyncProvider` reconciles to the server in the background. Loading state is `!isInitialized`. The only remaining task-data helper hook is `hooks/useTaskParentChain.ts`, which walks `parentId` chains for breadcrumbs.
