...
- **App State & Mutations**: Components consume tasks and task mutations from `useLocalState()` (e.g. `const { tasks, updateTask } = useLocalState()`) and settings from `useSettings()` (`const { settings, updateSettings } = useSettings()`). These are domain-scoped contexts, not pass-through wrappers — they exist for re-render isolation, so toggling a setting doesn't re-render the task list and vice versa. All mutations are synchronous local-first writes (no `useMutation`, no `isPending`); `SyncProvider` reconciles to the server in the background. Loading state is `!isInitialized` (aggregated across providers). The only remaining task-data helper hook is `hooks/useTaskParentChain.ts`, which walks `parentId` chains for breadcrumbs. Provider order in `App.tsx`: `SettingsProvider > LocalStateProvider > SyncProvider > ExpandedTasksProvider > DraftSessionProvider > TaskFormDialogProvider` (LocalState reads `settings.autoPinNewTasks` in `createTask`; Sync reads from both; DraftSession overlays on top of LocalState and is only mounted above the dialog subtree).
