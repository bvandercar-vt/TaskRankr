...
const checkTitleAndSubtasks = (task: TaskTreeNode) => {
  const cardTitle = cy
    .contains(`${TaskCard.CARD} ${TaskCard.TITLE}`, new RegExp(`^${task.name}$`))
    .should('exist')
  if (!task.subtasks?.length) return
  cardTitle.closest(TaskCard.CARD).find(TaskCard.EXPAND_BTN).click()
  cardTitle.closest(TaskCard.CARD).find(TaskCard.EXPAND_BTN).should('not.exist')
  cardTitle.closest(TaskCard.CARD).within(() => checkSubtasksInCard(task))
}
